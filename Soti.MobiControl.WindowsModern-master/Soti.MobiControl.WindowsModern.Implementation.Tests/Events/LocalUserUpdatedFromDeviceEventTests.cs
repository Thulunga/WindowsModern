...
{
    internal class LocalUserUpdatedFromDeviceEventTests
    {
        [Test]
        public void LocalUserUpdatedFromDeviceEvent_Test()
        {
            const int deviceId = 100001;
            var devId = Guid.NewGuid().ToString();
            const string userName = "UserName1";
            var testEvent = new LocalUserUpdatedFromDeviceEvent(deviceId, devId, userName);
